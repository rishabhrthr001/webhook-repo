...
# MongoDB connection
client = MongoClient(
    "mongodb+srv://rishabhrthr001:Bv7Bsr4g7Nbnc3a2@cluster0.qars1vk.mongodb.net/"
)
db = client["webhookDB"]
events = db["events"]
...
def get_events():
    ...
    for r in result:
        if r["type"] == "push":
            formatted.append(
                f'{r["author"]} pushed to {r["to_branch"]} on {format_time(r["timestamp"])}'
            )
        elif r["type"] == "pull_request":
            formatted.append(
                f'{r["author"]} submitted a pull request from {r["from_branch"]} to {r["to_branch"]} on {format_time(r["timestamp"])}'
            )
        elif r["type"] == "merge":
            formatted.append(
                f'{r["author"]} merged branch {r["from_branch"]} to {r["to_branch"]} on {format_time(r["timestamp"])}'
            )
